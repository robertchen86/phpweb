...
$kng_list = '';
$kng_list2 = '';
$kng_list2 .= '<option value="0">无</option>';
$kng_list2 .= get_kng_list2(0); 
$kng_list = '<option value="">全部</option>'.$kng_list2;
/***/
...
$sql.= ' order by a.video_kng_id desc, a.video_sort <> 0 desc, a.video_sort asc';
...
<script type="text/javascript" src="js/video_manage.js"></script>
...
<ul id="innouni_sub_title">
	...
	<li class="unsub"><a class="hand" onclick="$('#vout')[0].style.visibility='inherit'">视频导出</a></li>
</ul>
<div style="visibility:hidden;position:absolute;margin:0 40 40 170px;*margin:0 0 0 170px;border:1px solid #FCBA72;background:#fff;padding:5px 10px;" id="search">
	...
	<form method="post" action="./?a=video_to_manage">相关知识点：<select name="video_kng_id" id="video_kng_id"><?=$kng_list?></select>
...
<div style="visibility:hidden;position:absolute;margin:0 40 40 170px;*margin:0 0 0 170px;border:1px solid #FCBA72;background:#fff;padding:5px 10px;" id="vout">
	<!--<img src="images/close.gif"  style="position:absolute;margin:10px 0 0 612px;cursor:pointer;" onclick="$('#search')[0].style.visibility='hidden'" alt="关闭" />-->
	<form method="post" action="./?a=video_to_out">相关知识点：<select name="out_video_kng_id" id="out_video_kng_id"><?=$kng_list?></select>
		状态：<select name="out_video_td_state" id="out_video_td_state">
			<option value="">全部</option>
			<option value="1">正常播放</option>
			<option value="2">审核中</option>
			<option value="3">转码中</option>
		...
		关键字：<input name="out_keyword" id="out_keyword" class="input" value="" /> <input type="submit" class="bnt01" value="导出">
		<img src="images/close.gif"  style="margin:10px 0 0 10px;cursor:pointer;" onclick="$('#vout')[0].style.visibility='hidden'" alt="关闭"/>
	</form>
	
</div>
<div id="innouni_right_b">
<form action="./?a=video_to_manage&page=<?=$curpage?>&keyword=<?=$keyword ?>&video_kng_id=<?=$cur_kng_id ?>&video_td_state=<?=$cur_state?>" method="post" >
  ...
  <? foreach($result as $key => $value){ ?>
		<tr onmouseover=this.bgColor='#EBFFDC'; onmouseout=this.bgColor='#ffffff';  bgcolor='#ffffff'>
			...
			<td align="center">
				<a href="./?a=video_to_edit&page=<?=$curpage?>&keyword=<?=$keyword ?>&video_kng_id=<?=$cur_kng_id?>&video_td_state=<?=$cur_state?>&video_id=<?=$value['video_id']?>">编辑</a> 
				<a href="./?a=video_to_manage&page=<?=$curpage?>&keyword=<?=$keyword ?>&video_kng_id=<?=$cur_kng_id?>&video_td_state=<?=$cur_state?>&act=trash&video_id=<?=$value['video_id']?>" onclick="return confirm('确定要删除视频（<?=$value['video_title']?>）？');">删除</a>
			</td>
		</tr>
	<? } ?>
		<tr>
			<td colspan="10" class="tdbg" >
				...
				<select name="act"  id="act" disabled>
					...
					<option value="kng_change">关联转移</option>
				</select>
				<select name="to_kng"  id="to_kng" disabled><?=$kng_list2?></select>
				<input type="submit" class="bnt01" value="执行" id="dosubmit"  disabled />
			</td>
	  </tr>
  <? if ($totlepage > 1){ ?>
    <tr>
			<td colspan="10" align="center" class="tdbg">
				<? if($curpage <> 1){ ?>
				<a href="./?a=video_to_manage&page=<?=(int)($curpage - 1)?>&keyword=<?=$keyword ?>&video_kng_id=<?=$cur_kng_id ?>&video_td_state=<?=$cur_state?>">上一页</a>
				<? } ?>
				<?
				if($curpage <= 5){
					$start = 1;
				...
				?>
				<? if($start > 1){ ?>
				<a href="./?a=video_to_manage&page=1&keyword=<?=$keyword ?>&video_kng_id=<?=$cur_kng_id ?>&video_td_state=<?=$cur_state?>"><?=1?></a>...
				<? } ?>
				<? for($i = $start; $i <=$end; $i++){ ?>
					<? if($value<>$curpage){ ?>
					<a href="./?a=video_to_manage&page=<?=$i?>&keyword=<?=$keyword ?>&video_kng_id=<?=$cur_kng_id ?>&video_td_state=<?=$cur_state?>"><?=$i?></a>
					<? }else{ ?>
					<a><?=$value?></a>
					<? } ?>
				<? } ?>
				<? if($end < $totlepage){ ?>
				...<a href="./?a=video_to_manage&page=<?=$totlepage?>&keyword=<?=$keyword ?>&video_kng_id=<?=$cur_kng_id ?>&video_td_state=<?=$cur_state?>"><?=$totlepage?></a>
				<? } ?>
				<? if($curpage <> $totlepage){ ?>
				<a href="./?a=video_to_manage&page=<?=(int)($curpage + 1)?>&keyword=<?=$keyword ?>&video_kng_id=<?=$cur_kng_id ?>&video_td_state=<?=$cur_state?>">下一页</a>
				<? } ?>
