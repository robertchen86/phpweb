$(document).ready(function(){
	  ...
	  $("#member_identity_type").bind('change', function(){type_change();});
...
function type_change(){
	  if($("#member_identity_type").val() == 1){
	  	  $("#member_kng_id").attr('disabled',false);
	  }else{
	  	  $("#member_kng_id").attr('disabled',true);
	  }
	  school_change();
}
...
function checkName(){if(!checkSpace("member_name",0)){return false}$("#error-text").empty();return true}
function checkPassword(){if(!checkSpace("password",1)){return false}$("#error-text").empty();return true}
function checkRePassword(){if(!checkSpace("repassword",2)){return false}$("#error-text").empty();return true}
function checkBirthday(){var a=$("#birthyear").val()+"-"+$("#birthmonth").val()+"-"+$("#birthday").val();if(a!="--"){if(!isValidDate(a)){$("#error-text").empty();$("#error-text").append(errmsg[4]);return false}}$("#error-text").empty();return true}function isValidDate(a){var b=a.split("-");var c=new Date(b[0],(b[1]-1),b[2]);return !!(c&&(c.getMonth()+1)==b[1]&&c.getDate()==Number(b[2]))}
