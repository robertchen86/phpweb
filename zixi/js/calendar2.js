...
function CalendarInit()				///Create panel
{
    ...
    /// Month
    htmloMonth = "<select id='" + this.sMONTHID + "' onchange=CalendarMonthChange(" + this.instanceName + ") style='width:'" + this.Width*0.5 + ">";
    for (i = 0; i < 12; i++) {
        htmloMonth += "<option value='" + i + "'>" + this.monthSting[i] + "</option>";
    }
    htmloMonth += "</select>";
    /// Year
    htmloYear = "<select id='" + this.sYEARID + "' onchange=CalendarYearChange(" + this.instanceName + ") style='width:'" + this.Width*0.5 + ">";
    for (i = this.startYear; i <= this.currDate.getFullYear()+ 10; i++) {
        htmloYear += "<option value='" + i + "'>" + i + "</option>";
    }
    htmloYear += "</select></div>";
    /// Day
    htmloDayTable = "<table id='" + this.sTABLEID + "' width='" + this.Width + "' border=0 cellpadding=0 cellspacing=1 bgcolor='" + this.tableBorderColor + "'>";
    ...
    htmloButton += "<button style='width:" + (this.Width)*0.4+";border:1px solid #BCD0DE;background-color:#eeeeee;cursor:hand'"
