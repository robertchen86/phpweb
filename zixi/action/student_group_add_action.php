...
$cnt = $result[0]['cnt']; 
    ...
    $inrow = array(
        ...
        'm_group_t_id'=> $member_id,
        'm_group_addtime'=> date('Y-m-d H:i:s'),
        );
   $result = add_member_group($inrow);
	 if($result == false){
        $this->err_info = '分组添加失败！';
        $this->display('err.html');
        die();
   }
...
$this->title = '分组添加 - '.$web_name;
$this->keywords = '分组添加';
$this->description = '分组添加';
$this->display('student_group_add.html');
